...
from task import load_data, train, evaluate, get_weights, set_weights

# Usamos la misma ruta para info de cliente que en el experimento binario
...
class FlowerClient(Client):
    def __init__(
        ...
        # Propiedades usadas por el servidor para selección por Slice
        self.properties: Dict[str, int] = {
            "client_id": self.client_id,
            "Slice": self.slice_label,
        }

        # CSV de logging de info de cliente (una sola vez)
    ...
    def _log_client_info(self) -> None:
        """Guarda client_id y Slice en CSV (sobrescribe)."""
    ...
    def evaluate(self, ins: EvaluateIns) -> EvaluateRes:
        ...
        # 2) Evaluar loss y accuracy y log de clasificación en W&B
        loss, accuracy = evaluate(self.net, self.test_loader)

        # 3) Calcular métricas macro para las 7 clases
        all_preds = []
        all_labels = []
        self.net.eval()
        with torch.no_grad():
...
def client_fn(context) -> Client:
    """Factory que Flwr invoca por cliente."""
